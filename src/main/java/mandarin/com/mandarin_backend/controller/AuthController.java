...
public class AuthController {
    ...
    public LoginResponse login(@RequestBody LoginRequest request,
        ...
        if (user == null) {
            return new LoginResponse(false, "아이디 또는 비밀번호가 올바르지 않습니다.", 0);
        }

        session.setAttribute("LOGIN_USER_ID", user.getId());
        session.setAttribute("LOGIN_USER", user.getUserId());

        return new LoginResponse(true, "로그인에 성공했습니다.", user.getLoveType());
    }
    ...
    public LoginResponse me(HttpSession session) {
        ...
        if (id == null) {
            return new LoginResponse(false, "로그인되어 있지 않습니다.", 0);
        }

        return new LoginResponse(true, "현재 로그인된 유저 ID = " + id, 0);
    }

    @PostMapping("/logout")
    public LoginResponse logout(HttpSession session) {
        session.invalidate();
        return new LoginResponse(true, "로그아웃 되었습니다.", 0);
    }
}
