...
public class AuthController {
    ...
    public LoginResponse login(@RequestBody LoginRequest request,
        ...
        return new LoginResponse(true, "로그인에 성공했습니다.");
    }
    ...
    public LoginResponse me(HttpSession session) {
        ...
        if (id == null) {
            return new LoginResponse(false, "로그인되어 있지 않습니다.");
        }

        return new LoginResponse(true, "현재 로그인된 유저 ID = " + id);
    }

    @PostMapping("/logout")
    public LoginResponse logout(HttpSession session) {
        session.invalidate();
        return new LoginResponse(true, "로그아웃 되었습니다.");
    }
}
